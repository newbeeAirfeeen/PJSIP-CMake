 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_create(pj_pool_t *pool,
                  pj_size_t max_fd,
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_destroy(pj_ioqueue_t *ioqueue) {
    pj_ioqueue_key_t *key;
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_register_sock2(pj_pool_t *pool,
                          pj_ioqueue_t *ioqueue,
                          pj_sock_t sock,
...
PJ_DEF(pj_status_t)
pj_ioqueue_register_sock(pj_pool_t *pool,
                         pj_ioqueue_t *ioqueue,
                         pj_sock_t sock,
                         void *user_data,
                         const pj_ioqueue_callback *cb,
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_unregister(pj_ioqueue_key_t *key) {
    pj_ioqueue_t *ioqueue;
...
pj_ioqueue_poll(pj_ioqueue_t *ioqueue, const pj_time_val *timeout) {
    ...
    } __except (GetExceptionCode() == STATUS_INVALID_HANDLE ? EXCEPTION_CONTINUE_EXECUTION : EXCEPTION_CONTINUE_SEARCH) {
    }
#endif
    ...
    for (h = ioqueue->active_list.next;
        ...
        if ((key_has_pending_write(h) || key_has_pending_connect(h)) && PJ_FD_ISSET(h->fd, &wfdset) && !IS_CLOSING(h)) {
#if PJ_IOQUEUE_HAS_SAFE_UNREG
            increment_counter(h);
#endif
        ...
        if ((key_has_pending_read(h) || key_has_pending_accept(h)) && PJ_FD_ISSET(h->fd, &rfdset) && !IS_CLOSING(h) &&
