 ...
 */
PJ_DEF(pj_status_t)
pj_thread_register(const char *cstr_thread_name,
                   pj_thread_desc desc,
   ...
   */
    ...
    if (rc != PJ_SUCCESS) {
        __android_log_print(ANDROID_LOG_ERROR, "PJ", "pj_thread_local_set failed with error %d", rc);
...
pj_status_t pj_thread_init(void) {
    ...
    rc = pj_thread_register("thr%p", (long *) &main_thread, &dummy);
    return rc;
#else
 ...
 */
PJ_DEF(pj_status_t)
pj_thread_create(pj_pool_t *pool,
 ...
 */
PJ_DEF(const char *)
pj_thread_get_name(pj_thread_t *p) {
#if PJ_HAS_THREADS
 ...
 */
PJ_DEF(pj_status_t)
pj_thread_resume(pj_thread_t *p) {
    pj_status_t rc;
 ...
 */
PJ_DEF(pj_status_t)
pj_thread_destroy(pj_thread_t *p) {
    PJ_CHECK_STACK();
 ...
 */
PJ_DEF(void)
pj_thread_check_stack(const char *file, int line) {
    char stk_ptr;
    pj_uint32_t usage;
    pj_thread_t *thread = pj_thread_this();

    /* Calculate current usage. */
    usage = (&stk_ptr > thread->stk_start) ? &stk_ptr - thread->stk_start : thread->stk_start - &stk_ptr;
 ...
 */
PJ_DEF(pj_uint32_t)
pj_thread_get_stack_max_usage(pj_thread_t *thread) {
 ...
 */
PJ_DEF(pj_status_t)
pj_thread_get_stack_info(pj_thread_t *thread,
                         const char **file,
                         int *line) {
    pj_assert(thread);
 ...
 */
PJ_DEF(pj_status_t)
pj_atomic_create(pj_pool_t *pool,
 ...
 */
PJ_DEF(pj_status_t)
pj_atomic_destroy(pj_atomic_t *atomic_var) {
 ...
 */
PJ_DEF(void)
pj_atomic_set(pj_atomic_t *atomic_var, pj_atomic_value_t value) {
    PJ_CHECK_STACK();
...
pj_atomic_get(pj_atomic_t *atomic_var) {
 ...
 */
PJ_DEF(pj_atomic_value_t)
pj_atomic_inc_and_get(pj_atomic_t *atomic_var) {
 ...
 */
PJ_DEF(void)
pj_atomic_inc(pj_atomic_t *atomic_var) {
 ...
 */
PJ_DEF(void)
pj_atomic_dec(pj_atomic_t *atomic_var) {
 ...
 */
PJ_DEF(pj_atomic_value_t)
pj_atomic_add_and_get(pj_atomic_t *atomic_var,
                      pj_atomic_value_t value) {
    pj_atomic_value_t new_value;
 ...
 */
PJ_DEF(void)
pj_atomic_add(pj_atomic_t *atomic_var,
              pj_atomic_value_t value) {
 ...
 */
PJ_DEF(pj_status_t)
pj_thread_local_alloc(long *p_index) {
#if PJ_HAS_THREADS
    pthread_key_t key;
    int rc;
 ...
 */
PJ_DEF(void)
pj_thread_local_free(long index) {
    PJ_CHECK_STACK();
#if PJ_HAS_THREADS
 ...
 */
PJ_DEF(pj_status_t)
pj_thread_local_set(long index, void *value) {
...
PJ_DEF(void *)
pj_thread_local_get(long index) {
...
///////////////////////////////////////////////////////////////////////////////
PJ_DEF(void)
pj_enter_critical_section(void) {
#if PJ_HAS_THREADS
...
PJ_DEF(void)
pj_leave_critical_section(void) {
#if PJ_HAS_THREADS
 ...
 */
PJ_DEF(pj_status_t)
pj_mutex_create(pj_pool_t *pool,
                const char *name,
                int type,
 ...
 */
PJ_DEF(pj_status_t)
pj_mutex_create_simple(pj_pool_t *pool,
                       const char *name,
                       pj_mutex_t **mutex) {
 ...
 */
PJ_DEF(pj_status_t)
pj_mutex_create_recursive(pj_pool_t *pool,
                          const char *name,
                          pj_mutex_t **mutex) {
 ...
 */
PJ_DEF(pj_status_t)
pj_mutex_lock(pj_mutex_t *mutex) {
#if PJ_HAS_THREADS
    pj_status_t status;
    ...
    PJ_LOG(6, (mutex->obj_name,
               (status == 0 ? "Mutex acquired by thread %s (level=%d)" : "Mutex acquisition FAILED by %s (level=%d)"),
               pj_thread_this()->obj_name,
               mutex->nesting_level));
#else
    PJ_LOG(6, (mutex->obj_name,
 ...
 */
PJ_DEF(pj_status_t)
pj_mutex_unlock(pj_mutex_t *mutex) {
#if PJ_HAS_THREADS
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pj_mutex_trylock(pj_mutex_t *mutex) {
#if PJ_HAS_THREADS
    int status;
 ...
 */
PJ_DEF(pj_status_t)
pj_mutex_destroy(pj_mutex_t *mutex) {
    enum { RETRY = 4 };
    int status = 0;
    unsigned retry;
...
#if PJ_DEBUG
PJ_DEF(pj_bool_t)
pj_mutex_is_locked(pj_mutex_t *mutex) {
#if PJ_HAS_THREADS
...
PJ_DEF(pj_status_t)
pj_rwmutex_create(pj_pool_t *pool, const char *name,
 ...
 */
PJ_DEF(pj_status_t)
pj_rwmutex_lock_read(pj_rwmutex_t *mutex) {
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pj_rwmutex_lock_write(pj_rwmutex_t *mutex) {
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pj_rwmutex_unlock_read(pj_rwmutex_t *mutex) {
 ...
 */
PJ_DEF(pj_status_t)
pj_rwmutex_unlock_write(pj_rwmutex_t *mutex) {
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pj_rwmutex_destroy(pj_rwmutex_t *mutex) {
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pj_event_create(pj_pool_t *pool, const char *name,
 ...
 */
PJ_DEF(pj_status_t)
pj_event_wait(pj_event_t *event) {
    PJ_UNUSED_ARG(event);
    return PJ_EINVALIDOP;
}
 ...
 */
PJ_DEF(pj_status_t)
pj_event_trywait(pj_event_t *event) {
    PJ_UNUSED_ARG(event);
    return PJ_EINVALIDOP;
}
 ...
 */
PJ_DEF(pj_status_t)
pj_event_pulse(pj_event_t *event) {
    PJ_UNUSED_ARG(event);
    return PJ_EINVALIDOP;
}
 ...
 */
PJ_DEF(pj_status_t)
pj_event_reset(pj_event_t *event) {
    PJ_UNUSED_ARG(event);
    return PJ_EINVALIDOP;
}
 ...
 */
PJ_DEF(pj_status_t)
pj_event_destroy(pj_event_t *event) {
    PJ_UNUSED_ARG(event);
    return PJ_EINVALIDOP;
}
#endif /* PJ_HAS_EVENT_OBJ */
 ...
 */
PJ_DEF(pj_status_t)
pj_term_set_color(pj_color_t color) {
