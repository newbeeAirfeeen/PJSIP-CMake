...
PJ_INLINE(void)
lock_timer_heap(pj_timer_heap_t *ht) {
    if (ht->lock) {
...
PJ_INLINE(void)
unlock_timer_heap(pj_timer_heap_t *ht) {
    if (ht->lock) {
...
PJ_DEF(unsigned)
pj_timer_heap_set_max_timed_out_per_poll(pj_timer_heap_t *ht,
                                         unsigned count) {
...
#if PJ_TIMER_DEBUG
PJ_DEF(pj_status_t)
pj_timer_heap_schedule_dbg(pj_timer_heap_t *ht,
                           pj_timer_entry *entry,
                           const pj_time_val *delay,
                           const char *src_file,
                           int src_line) {
...
PJ_DEF(pj_status_t)
pj_timer_heap_schedule_w_grp_lock(pj_timer_heap_t *ht,
                                  pj_timer_entry *entry,
                                  const pj_time_val *delay,
                                  int id_val,
                                  pj_grp_lock_t *grp_lock) {
...
PJ_DEF(int)
pj_timer_heap_cancel_if_active(pj_timer_heap_t *ht,
                               pj_timer_entry *entry,
                               int id_val) {
...
PJ_DEF(pj_size_t)
pj_timer_heap_count(pj_timer_heap_t *ht) {
    PJ_ASSERT_RETURN(ht, 0);
...
PJ_DEF(pj_status_t)
pj_timer_heap_earliest_time(pj_timer_heap_t *ht,
                            pj_time_val *timeval) {
...
#if PJ_TIMER_DEBUG
PJ_DEF(void)
pj_timer_heap_dump(pj_timer_heap_t *ht) {
    lock_timer_heap(ht);
