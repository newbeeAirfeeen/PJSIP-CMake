 ...
 */
PJ_DEF(void)
pj_ice_strans_stun_cfg_default(pj_ice_strans_stun_cfg *cfg) {
    pj_bzero(cfg, sizeof(*cfg));
 ...
 */
PJ_DEF(void)
pj_ice_strans_turn_cfg_default(pj_ice_strans_turn_cfg *cfg) {
    pj_bzero(cfg, sizeof(*cfg));
 ...
 */
PJ_DEF(void)
pj_ice_strans_cfg_copy(pj_pool_t *pool,
...
static pj_bool_t ice_cand_equals(pj_ice_sess_cand *lcand,
    ...
    if (lcand->type != rcand->type || lcand->status != rcand->status || lcand->comp_id != rcand->comp_id || lcand->transport_id != rcand->transport_id || lcand->local_pref != rcand->local_pref || lcand->prio != rcand->prio || pj_sockaddr_cmp(&lcand->addr, &rcand->addr) != 0 || pj_sockaddr_cmp(&lcand->base_addr, &rcand->base_addr) != 0) {
        return PJ_FALSE;
    }
...
static pj_status_t add_stun_and_host(pj_ice_strans *ice_st,
    ...
    if (stun_cfg->server.slen) {
        ...
        if (comp->cand_list[comp->default_cand].type != PJ_ICE_CAND_TYPE_SRFLX || (ice_st->cfg.af != pj_AF_UNSPEC() &&
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_get_options(pj_ice_strans *ice_st,
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_set_options(pj_ice_strans *ice_st,
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_init_ice(pj_ice_strans *ice_st,
     ...
     * checked first.
     */
    if (ice_st->comp[0]->cand_list[ice_st->comp[0]->default_cand].type == PJ_ICE_CAND_TYPE_SRFLX) {
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_get_ufrag_pwd(pj_ice_strans *ice_st,
                            pj_str_t *loc_ufrag,
                            pj_str_t *loc_pwd,
                            pj_str_t *rem_ufrag,
                            pj_str_t *rem_pwd) {
 ...
 */
PJ_DEF(unsigned)
pj_ice_strans_get_cands_count(pj_ice_strans *ice_st,
                              unsigned comp_id) {
    unsigned i, cnt;

    PJ_ASSERT_RETURN(ice_st && ice_st->ice && comp_id &&
                             comp_id <= ice_st->comp_cnt,
                     0);
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_enum_cands(pj_ice_strans *ice_st,
                         unsigned comp_id,
                         unsigned *count,
                         pj_ice_sess_cand cand[]) {
    unsigned i, cnt;
    ...
    PJ_ASSERT_RETURN(ice_st && ice_st->ice && comp_id &&
                             comp_id <= ice_st->comp_cnt && count && cand,
                     PJ_EINVAL);
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_get_def_cand(pj_ice_strans *ice_st,
                           unsigned comp_id,
                           pj_ice_sess_cand *cand) {
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_change_role(pj_ice_strans *ice_st,
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_strans_sendto(pj_ice_strans *ice_st,
                     unsigned comp_id,
                     const void *data,
                     pj_size_t data_len,
                     const pj_sockaddr_t *dst_addr,
        ...
        case PJ_STUN_SOCK_MAPPED_ADDR_CHANGE:
            if (status == PJ_SUCCESS) {
                ...
                if (status == PJ_SUCCESS) {
                    ...
                    const char *op_name = (op == PJ_STUN_SOCK_BINDING_OP) ? "Binding discovery complete" : "srflx address changed";
...
static void turn_on_state(pj_turn_sock *turn_sock, pj_turn_state_t old_state,
    ...
    if (new_state == PJ_TURN_STATE_READY) {
        ...
        if (comp->cand_list[comp->default_cand].type != PJ_ICE_CAND_TYPE_RELAYED || (comp->ice_st->cfg.af != pj_AF_UNSPEC() &&
                                                                                     comp->cand_list[comp->default_cand].addr.addr.sa_family != comp->ice_st->cfg.af)) {
            comp->default_cand = (unsigned) (cand - comp->cand_list);
        }
