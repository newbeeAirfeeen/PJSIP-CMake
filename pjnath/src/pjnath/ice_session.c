 ...
 */
PJ_DEF(void)
pj_ice_calc_foundation(pj_pool_t *pool,
                       pj_str_t *foundation,
...
PJ_DEF(void)
pj_ice_sess_options_default(pj_ice_sess_options *opt) {
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_sess_create(pj_stun_config *stun_cfg,
                   const char *name,
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_sess_get_options(pj_ice_sess *ice,
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_sess_set_options(pj_ice_sess *ice,
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_sess_change_role(pj_ice_sess *ice,
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_sess_set_prefs(pj_ice_sess *ice,
 ...
 */
PJ_DEF(pj_status_t)
pj_ice_sess_add_cand(pj_ice_sess *ice,
                     unsigned comp_id,
...
PJ_DEF(pj_status_t)
pj_ice_sess_find_default_cand(pj_ice_sess *ice,
                              unsigned comp_id,
                              int *cand_id) {
    unsigned i;
...
PJ_INLINE(int)
CMP_CHECK_PRIO(const pj_ice_sess_check *c1,
...
static void ice_keep_alive(pj_ice_sess *ice, pj_bool_t send_now) {
    ...
    if (ice->timer.id == TIMER_NONE) {
        ...
        delay.msec = (PJ_ICE_SESS_KEEP_ALIVE_MIN +
                      (pj_rand() % PJ_ICE_SESS_KEEP_ALIVE_MAX_RAND)) *
                     1000 /
                     ice->comp_cnt;
...
static pj_bool_t on_check_complete(pj_ice_sess *ice,
    ...
    if (check->err_code == PJ_SUCCESS) {
        ...
        for (i = 0; i < ice->clist.count; ++i) {
            ...
            if (pj_strcmp(&c->lcand->foundation, &check->lcand->foundation) == 0 && c->state == PJ_ICE_SESS_CHECK_STATE_FROZEN) {
    ...
    if (check->err_code == PJ_SUCCESS && check->nominated) {
        ...
        for (i = 0; i < ice->clist.count; ++i) {
            ...
            if (c->lcand->comp_id == check->lcand->comp_id) {
                ...
                } else if (c->state == PJ_ICE_SESS_CHECK_STATE_IN_PROGRESS && (PJ_ICE_CANCEL_ALL ||
...
PJ_DEF(pj_status_t)
pj_ice_sess_send_data(pj_ice_sess *ice,
                      unsigned comp_id,
                      const void *data,
                      pj_size_t data_len) {
    pj_status_t status = PJ_SUCCESS;
...
PJ_DEF(pj_status_t)
pj_ice_sess_on_rx_pkt(pj_ice_sess *ice,
                      unsigned comp_id,
                      unsigned transport_id,
                      void *pkt,
                      pj_size_t pkt_size,
