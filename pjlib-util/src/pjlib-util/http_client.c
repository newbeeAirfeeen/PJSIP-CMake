...
static pj_bool_t http_on_data_read(pj_activesock_t *asock,
        ...
        if (st == PJLIB_UTIL_EHTTPINCHDR) {
            ...
            *remainder = size;
            ...
            if (rem > 0 || hreq->response.content_length == 0)
                return http_on_data_read(asock, (rem == 0 ? NULL : (char *) data + size - rem),
                                         rem, PJ_SUCCESS, NULL);
        }
    ...
    if (hreq->cb.on_data_read) {
        ...
        if (hreq->response.size == 0) {
             ...
             */
            hreq->response.size = (hreq->response.content_length == -1 ? INITIAL_DATA_BUF_SIZE : hreq->response.content_length);
        ...
        pj_memcpy((char *) hreq->response.data +
                          hreq->tcp_state.current_read_size,
                  data, size);
    }
    ...
    if ((status != PJ_SUCCESS && status != PJ_EPENDING && status != PJ_EEOF) || (status == PJ_EEOF && hreq->response.content_length > -1)) {
        hreq->error = status;
        pj_http_req_cancel(hreq, PJ_TRUE);
        return PJ_FALSE;
    }
 ...
 */
PJ_DEF(pj_status_t)
pj_http_headers_add_elmt2(pj_http_headers *headers,
                          char *name, char *val) {
    pj_str_t f, v;
    pj_cstr(&f, name);
    pj_cstr(&v, val);
...
PJ_DEF(pj_status_t)
pj_http_headers_add_elmt(pj_http_headers *headers,
                         pj_str_t *name,
                         pj_str_t *val) {
...
PJ_DEF(void)
pj_http_req_param_default(pj_http_req_param *param) {
    pj_assert(param);
...
PJ_DEF(pj_status_t)
pj_http_req_parse_url(const pj_str_t *url,
                      pj_http_url *hurl) {
    pj_scanner scanner;
...
PJ_DEF(void)
pj_http_req_set_timeout(pj_http_req *http_req,
                        const pj_time_val *timeout) {
    pj_memcpy(&http_req->param.timeout, timeout, sizeof(*timeout));
}

PJ_DEF(pj_status_t)
pj_http_req_create(pj_pool_t *pool,
                   const pj_str_t *url,
    ...
    *http_req = NULL;
    ...
    if (param) {
        ...
        PJ_ASSERT_RETURN(hreq->param.addr_family == PJ_AF_UNSPEC ||
                                 ...
                                 hreq->param.addr_family == PJ_AF_INET6,
                         PJ_EAFNOTSUP);
...
PJ_DEF(pj_bool_t)
pj_http_req_is_running(const pj_http_req *http_req) {
    PJ_ASSERT_RETURN(http_req, PJ_FALSE);
    return (http_req->state != IDLE);
}

PJ_DEF(void *)
pj_http_req_get_user_data(pj_http_req *http_req) {
...
static pj_status_t start_http_req(pj_http_req *http_req,
	 ...
	 */
        if (http_req->param.source_port_range_start != 0) {
            port = (pj_uint16_t) (http_req->param.source_port_range_start +
...
PJ_DEF(pj_status_t)
pj_http_req_start(pj_http_req *http_req) {
...
static pj_status_t http_req_start_sending(pj_http_req *hreq) {
    ...
    if (hreq->state == SENDING_REQUEST) {
        ...
        if (!pj_strcmp2(&hreq->param.method, http_method_names[HTTP_PUT])) {
            ...
            pj_utoa(hreq->param.reqdata.total_size ? (unsigned long) hreq->param.reqdata.total_size : (unsigned long) hreq->param.reqdata.size, buf);
...
PJ_DEF(pj_status_t)
pj_http_req_cancel(pj_http_req *http_req,
                   pj_bool_t notify) {
    ...
    if (notify && http_req->cb.on_complete) {
        (*http_req->cb.on_complete)(http_req, (!http_req->error ? PJ_ECANCELLED : http_req->error), NULL);
    }
...
PJ_DEF(pj_status_t)
pj_http_req_destroy(pj_http_req *http_req) {
    PJ_ASSERT_RETURN(http_req, PJ_EINVAL);
