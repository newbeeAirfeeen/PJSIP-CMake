 ...
 */
PJ_DEF(pj_status_t)
pjmedia_ice_create(pjmedia_endpt *endpt,
                   const char *name,
                   unsigned comp_cnt,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_ice_create2(pjmedia_endpt *endpt,
                    const char *name,
                    unsigned comp_cnt,
                    const pj_ice_strans_cfg *cfg,
                    const pjmedia_ice_cb *cb,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_ice_create3(pjmedia_endpt *endpt,
                    const char *name,
                    unsigned comp_cnt,
                    const pj_ice_strans_cfg *cfg,
                    const pjmedia_ice_cb *cb,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_ice_add_ice_cb(pjmedia_transport *tp,
                       const pjmedia_ice_cb *cb,
                       void *user_data) {
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_ice_remove_ice_cb(pjmedia_transport *tp,
                          const pjmedia_ice_cb *cb,
                          void *user_data) {
    struct transport_ice *tp_ice = (struct transport_ice *) tp;
    ice_listener *il;
...
static pj_status_t transport_media_create(pjmedia_transport *tp,
     ...
     */
    ice_role = (rem_sdp == NULL ? PJ_ICE_SESS_ROLE_CONTROLLING : PJ_ICE_SESS_ROLE_CONTROLLED);
        ...
        if (!tp_ice->use_ice) {
		     ...
		     */
                    if (!pj_sockaddr_has_addr(&tp_ice->rtcp_src_addr)) {
                        ...
                        port = (pj_uint16_t) (pj_sockaddr_get_port(&tp_ice->remote_rtp) + 1);
...
static void ice_on_ice_complete(pj_ice_strans *ice_st,
    ...
    pj_perror(5, tp_ice->base.name, result, "ICE operation complete"
                                            " (op=%d%s)",
              op,
              (op == PJ_ICE_STRANS_OP_INIT ? "/initialization" : (op == PJ_ICE_STRANS_OP_NEGOTIATION ? "/negotiation" : "")));
